...
impl BoundedValue {
    ...
    fn push_str(&mut self, value: &str) {
        ...
        if self.prefix.len() < MAX_PROJECT_IDENTITY_BYTES {
            let remaining = MAX_PROJECT_IDENTITY_BYTES - self.prefix.len();
            let mut end = value.len().min(remaining);
            while !value.is_char_boundary(end) {
                end -= 1;
            }
            self.prefix.push_str(&value[..end]);
        }
    }
    ...
    fn finish_with_prefix(mut self, identity_prefix: &str) -> String {
        if identity_prefix.len() + self.original_bytes <= MAX_PROJECT_IDENTITY_BYTES {
