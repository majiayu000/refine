...
#[tokio::test]
async fn body_detected_looper_clears_exact_version_quarantine_before_summary_skip() {
    let store = Arc::new(SqliteStore::in_memory().expect("in-memory sqlite store"));
    let doc_store: Arc<dyn DocumentRepository> = store.clone();
    let item_store: Arc<dyn ItemRepository> = store.clone();
    // Truncated summary samples must not let exact-version quarantine skip the load.
    let mut looper = remem_summary("looper", 300, 'c');
    looper.user_message_samples = vec!["Follow the daily checklist and report status.".to_string()];
    let mut eligible = remem_summary("eligible", 200, 'd');
    eligible.session_mode = "unattended".to_string();
    ...
    .await
    .expect("body-detected Looper must clear exact-version quarantine before summary skip");
    ...
    assert!(!item_store
        ...
        .is_empty());
    let quarantine = QuarantineStore::load_from(quarantine_path).unwrap();
    assert!(!quarantine.contains(stable.url(), Some(stable.source_version().unwrap())));
    assert_eq!(quarantine.len(), 0);
}

#[tokio::test]
async fn body_detected_looper_clears_skip_unchanged_stable_before_summary_skip() {
    let store = Arc::new(SqliteStore::in_memory().expect("in-memory sqlite store"));
    let doc_store: Arc<dyn DocumentRepository> = store.clone();
    let item_store: Arc<dyn ItemRepository> = store.clone();
    // No legacy candidate: skip_unchanged would return true on summary alone.
    let mut looper = remem_summary("looper", 300, 'c');
    looper.user_message_samples = vec!["Follow the daily checklist and report status.".to_string()];
    let mut eligible = remem_summary("eligible", 200, 'd');
    eligible.session_mode = "unattended".to_string();
    ...
    .await
    .expect("body-detected Looper must clear skip_unchanged stable before summary skip");
    ...
    assert!(!item_store
        ...
        .unwrap()
        .is_empty());
    let _ = QuarantineStore::load_from(quarantine_path).unwrap();
}
